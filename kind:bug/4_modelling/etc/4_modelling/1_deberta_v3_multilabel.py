...
class DeBERTaV3Classifier(nn.Module):
    """
    A classifier model based on DeBERTa V3 for multi-label classification.
    
    This model uses a pre-trained DeBERTa V3 model as the encoder and adds a 
    ...
    Args:
        ...
        model_type (str): Type of DeBERTa V3 model to use ('base' or 'large').
    """
    def __init__(self, num_labels, model_type='base'):
        super().__init__()
        ...
        if model_type not in model_paths:
            raise ValueError(f"Model type must be one of {list(model_paths.keys())}")
            
        self.model_type = model_type
        self.deberta = AutoModel.from_pretrained(model_paths[model_type])
        
        # Get hidden size based on model type
        hidden_size = 768 if model_type == 'base' else 1024
        ...
        # Note: DeBERTa V3 has a different architecture than V2
    ...
    def forward(self, input_ids, attention_mask):
        ...
        # DeBERTa V3 doesn't have a pooler, so we need to take the last hidden state
...
def main(args):
    """
    Main function to run the multi-label classification pipeline with DeBERTa V3.
    ...
    run_dir = os.path.join(results_dir, f"run_{timestamp}_{args.text_column}_{args.model_type}")
    ...
    tokenizer = AutoTokenizer.from_pretrained(model_paths[args.model_type])
    ...
    model = DeBERTaV3Classifier(num_labels=len(mlb.classes_), model_type=args.model_type).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        model_path = os.path.join(run_dir, f'best_model_{args.text_column}_{args.model_type}.pt')
    ...
    best_model_path = os.path.join(run_dir, f'best_model_{args.text_column}_{args.model_type}.pt')
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title(f'Training History - {args.text_column} - {args.model_type}')
    plt.legend()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train DeBERTa V3 for multi-label classification')
    ...
    parser.add_argument('--model_type', type=str, default='base',
                        choices=['base', 'large'],
                        help='Type of DeBERTa V3 model to use (base or large)')
