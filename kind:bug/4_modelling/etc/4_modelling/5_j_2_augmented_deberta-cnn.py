...
def main(args):
    ...
    if 'labels' in df.columns:
        try:
            df['labels'] = df['labels'].apply(
                lambda x: eval(x) if isinstance(x, str) and x.strip() not in ('', 'nan', 'None', 'NaN') else ([] if pd.isna(x) else x)
            )
        except Exception as e:
            print(f"Error processing labels: {e}")
            print("First few labels for inspection:", df['labels'].head())
            raise
    else:
        raise ValueError(f"'labels' column not found in data. Available columns: {df.columns.tolist()}")
    # Check if the text column exists
    if args.text_column not in df.columns:
        raise ValueError(f"Text column '{args.text_column}' not found in the data file.")
    # Split into train/test
    from sklearn.model_selection import train_test_split
    train_df, test_df = train_test_split(df, test_size=args.test_size if hasattr(args, 'test_size') else 0.2, random_state=42, shuffle=True)
    ...
    print(f"Initial training dataset size: {len(df) - len(test_df)}")
    print(f"Final training dataset size: {len(train_texts)}")
    print(f"Total samples removed: {(len(df) - len(test_df)) - len(train_texts)} ({((len(df) - len(test_df)) - len(train_texts))/(len(df) - len(test_df))*100 if (len(df) - len(test_df)) > 0 else 0:.2f}% of original training data)")
    ...
    print(f"Total samples removed: {len(test_df) - len(test_texts)} ({(len(test_df) - len(test_texts))/len(test_df)*100 if len(test_df) > 0 else 0:.2f}% of original testing data)")
    ...
    preprocessing_metadata = {
        ...
        # 'predefined_labels' removed; hybrid feature selection is now used
    }
    ...
    # --- Hybrid Feature Selection (from 5f) ---
    ...
    def hybrid_feature_selection(texts, labels_encoded, mlb, top_k_filter=20, top_k_final=10, vectorizer=None, random_seed=42, wrapper_method='rf'):
        ...
        for i in range(labels_encoded.shape[1]):
            try:
                chi2_score = chi2(X_vec, labels_encoded[:, i])[0].mean()
            except Exception:
                chi2_score = 0
            try:
                mi_score = mutual_info_classif(X_vec, labels_encoded[:, i], random_state=random_seed)
                mi_score = np.mean(mi_score)
            except Exception:
                mi_score = 0
            feature_scores[i] = chi2_score + mi_score
        ...
        print(f"Filter stage selected {len(filter_selected_indices)} labels: {filter_selected_labels}")
        if wrapper_method.lower() == 'rf':
            model = RandomForestClassifier(n_estimators=100, random_state=random_seed)
        elif wrapper_method.lower() == 'lr':
            model = LogisticRegression(max_iter=1000, random_state=random_seed)
        else:
            raise ValueError(f"Unknown wrapper_method: {wrapper_method}")
        X_filtered = labels_encoded[:, filter_selected_indices]
        model.fit(X_filtered, labels_encoded)
        importances = np.mean([tree.feature_importances_ for tree in model.estimators_], axis=0) if wrapper_method.lower() == 'rf' else np.abs(model.coef_).mean(axis=0)
        top_final_indices = np.argsort(-importances)[:top_k_final]
        selected_indices = filter_selected_indices[top_final_indices]
        selected_labels = np.array(mlb.classes_)[selected_indices]
        print(f"Hybrid feature selection chose {len(selected_labels)} labels: {selected_labels}")
        return selected_indices, selected_labels, importances[top_final_indices]

    # Run hybrid feature selection
    top_k_filter = 20
    top_k_final = 10
    selected_indices, selected_labels, _ = hybrid_feature_selection(train_texts, train_labels_encoded, mlb, top_k_filter=top_k_filter, top_k_final=top_k_final)
    ...
    with open(os.path.join(run_dir, 'selected_labels.json'), 'w') as f:
        json.dump({
            ...
            'selection_method': 'hybrid_feature_selection',
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path', type=str, 
                        ...
                        help='Path to the single CSV file containing all data (will be split into train/test)')
