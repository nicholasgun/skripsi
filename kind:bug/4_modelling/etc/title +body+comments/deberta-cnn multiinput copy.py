...
class CodeBERTCNNClassifier(nn.Module):
    """
    A hybrid classifier model based on CodeBERT and CNN for multi-label classification.
    
    This model uses a pre-trained CodeBERT model as the encoder to generate embeddings,
...
DeBERTaClassifier = CodeBERTCNNClassifier
DeBERTaCNNClassifier = CodeBERTCNNClassifier
...
def main(args):
    ...
    run_dir = os.path.join(results_dir, f"codebert_cnn_run_{timestamp}_{args.text_column}")
    ...
    # Initialize CodeBERT-CNN model
    model = CodeBERTCNNClassifier(
        num_labels=len(mlb.classes_),
        filter_sizes=filter_sizes,
        num_filters=args.cnn_num_filters
    ).to(device)
    
    print(f"Initialized CodeBERT-CNN model with {len(filter_sizes)} filter sizes and {args.cnn_num_filters} filters per size")
    ...
    for epoch in range(num_epochs):
        ...
        model_path = os.path.join(run_dir, f'best_model_codebert_cnn_{args.text_column}.pt')
    ...
    best_model_path = os.path.join(run_dir, f'best_model_codebert_cnn_{args.text_column}.pt')
