...
class DeBERTaClassifier(nn.Module):
    """
    A classifier model based on DeBERTa with PCA for token dimension reduction.
    
    This model uses a pre-trained DeBERTa model as the encoder, applies PCA to reduce
    the dimensionality of token embeddings, and adds a classification head on top
    with sigmoid activation for multi-label output.
    
    Args:
        num_labels (int): Number of classes in the multi-label classification task.
        pca_components (int, optional): Number of PCA components to use. If None, PCA is not used.
        Default is 128.
    """
    def __init__(self, num_labels, pca_components=128):
        super().__init__()
        ...
        # Set the classifier based on whether PCA is used
        if pca_components is None:
            self.classifier = nn.Linear(768, num_labels)
        else:
            self.classifier = nn.Linear(pca_components, num_labels)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        # Apply PCA dimension reduction if enabled
        if self.pca_components is not None:
            # Initialize PCA with the first batch if not already initialized
            if not self.pca_initialized:
                with torch.no_grad():
                    # Move to CPU for scikit-learn PCA
                    cpu_embeddings = cls_output.detach().cpu().numpy()
                    self.pca = PCA(n_components=self.pca_components)
                    self.pca.fit(cpu_embeddings)
                    self.pca_initialized = True
                    print(f"PCA initialized with {self.pca_components} components, "
                          f"explaining {self.pca.explained_variance_ratio_.sum():.2f} of variance")
            
            # Transform embeddings using PCA
            with torch.no_grad():
                cpu_embeddings = cls_output.detach().cpu().numpy()
                reduced_embeddings = self.pca.transform(cpu_embeddings)
                # Move back to GPU
                cls_output = torch.tensor(reduced_embeddings, 
                                          dtype=torch.float32, 
...
def main(args):
    ...
    # Set pca_components to None if 0 is specified to disable PCA
    ...
    if pca_components is not None:
        print(f"Using PCA with {pca_components} components for token dimension reduction")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pca_components', type=int, default=128, 
                      help='Number of PCA components to use for token dimension reduction. Set to 0 to disable PCA.')
