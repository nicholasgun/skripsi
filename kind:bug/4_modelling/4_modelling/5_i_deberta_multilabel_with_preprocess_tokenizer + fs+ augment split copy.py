...
def main(args):
    ...
    # Explicitly calculate token lengths for metadata
    ...
    if args.feature_selection:
        ...
        # Update the number of labels for the model
        num_labels = len(selected_indices)
        print(f"Updated number of labels after feature selection: {num_labels}")
    else:
        num_labels = len(mlb.classes_)
        print(f"Number of labels (no feature selection): {num_labels}")
        # Define selected_labels for non-feature selection case to avoid errors
        selected_labels = mlb.classes_
    
    # Now create preprocessing metadata after mlb and feature selection have been defined
    preprocessing_metadata = {
        ...
        'feature_selection': {
            'applied': args.feature_selection,
            'filter_k': args.filter_k if args.feature_selection else None,
            'final_k': args.final_k if args.feature_selection else None,
            'vectorizer': args.vectorizer if args.feature_selection else None,
            'wrapper_method': args.wrapper_method if args.feature_selection else None,
            'original_label_count': len(mlb.classes_),
            'selected_label_count': len(selected_labels) if args.feature_selection else len(mlb.classes_)
        }
    }
    
    # Add token statistics to metadata
    ...
    if args.use_class_weights:
        # If we're using feature selection, adjust label_counts to match the selected labels
        if args.feature_selection:
            label_counts = train_labels_encoded.sum(axis=0)
        
        # Apply weights if there's significant imbalance
        if label_counts.min() < label_counts.max() / 5:
    ...
    model = DeBERTaClassifier(num_labels=num_labels).to(device)
    ...
    # We use selected_labels which is already properly defined for both cases
    num_labels_to_plot = min(20, len(selected_labels))
    
    fig = plot_multilabel_confusion_matrix(
        all_test_labels[:, :num_labels_to_plot], 
        all_preds[:, :num_labels_to_plot], 
        selected_labels[:num_labels_to_plot]
    )
