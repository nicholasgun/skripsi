...
# Add a helper function for random oversampling (will be used later)
def random_oversample(texts, labels):
    """
    Performs random oversampling for multi-label data.
    Duplicates samples from minority classes until all classes have
    roughly the same number of samples as the majority class.
    """
    print("Applying Random Oversampling...")
    # Find the maximum frequency
    label_counts_per_sample = labels.sum(axis=0)
    if len(label_counts_per_sample) == 0 or label_counts_per_sample.max() == 0:
        print("No labels found or max count is 0, skipping oversampling.")
        return texts, labels

    max_count = label_counts_per_sample.max()
    n_samples, n_labels = labels.shape

    resampled_indices = list(range(n_samples))
    original_indices_set = set(range(n_samples))

    for label_idx in range(n_labels):
        current_count = label_counts_per_sample[label_idx]
        if current_count == 0: continue # Skip labels with no samples

        n_to_add = max_count - current_count
        if n_to_add <= 0: continue

        # Get indices of samples having this label
        label_sample_indices = np.where(labels[:, label_idx] == 1)[0]

        # Randomly choose indices to duplicate (with replacement)
        chosen_indices = np.random.choice(label_sample_indices, size=n_to_add, replace=True)
        resampled_indices.extend(chosen_indices)

    print(f"Original training size: {n_samples}")
    print(f"Size after random oversampling: {len(resampled_indices)}")

    # Ensure texts is indexed correctly if it's a Series
    if isinstance(texts, pd.Series):
        resampled_texts = texts.iloc[resampled_indices].reset_index(drop=True)
    else: # Assuming list or numpy array
         # Need numpy array for advanced indexing
        texts_np = np.array(texts)
        resampled_texts = texts_np[resampled_indices]

    resampled_labels = labels[resampled_indices]

    # Verify new counts (optional)
    new_counts = resampled_labels.sum(axis=0)
    print(f"Label counts after oversampling: {new_counts}")
...
def main(args):
    ...
    if args.balancing_strategy == 'tfidf_smote':
        print("Applying TF-IDF + MLSMOTE guided oversampling (Experimental)...")
        if len(train_texts) == 0:
             print("Warning: train_texts is empty. Skipping SMOTE.")
        else:
            try:
                # 1. Create TF-IDF Vectors for training data
                print("Creating TF-IDF vectors for training data...")
                vectorizer = TfidfVectorizer(max_features=10000, stop_words='english') # Limit features for performance
                X_train_tfidf = vectorizer.fit_transform(train_texts)
                print(f"TF-IDF matrix shape: {X_train_tfidf.shape}")

                # 2. Apply MLSMOTE
                # MLSMOTE requires k_neighbors < number of samples in smallest class group
                # Calculate minimum samples per label combination for safety
                label_sums = train_labels.sum(axis=1)
                min_samples_per_label = float('inf')
                if len(np.unique(label_sums)) > 1: # Check if there are multiple label combinations
                    # Count samples for each label combination
                    unique_labels, counts = np.unique(train_labels, axis=0, return_counts=True)
                    if len(counts) > 0:
                        min_samples_per_label = counts.min()

                # Adjust k_neighbors if necessary, minimum 1
                k_neighbors = min(5, max(1, min_samples_per_label - 1)) if min_samples_per_label > 1 else 1
                print(f"Using k_neighbors={k_neighbors} for MLSMOTE")

                print("Applying MLSMOTE...")
                # Ensure train_labels is float for MLSMOTE (sometimes required)
                train_labels_float = train_labels.astype(float)
                mlsmote = MLSMOTE(k_neighbors=k_neighbors, random_state=42, n_jobs=-1)
                X_resampled_tfidf, y_resampled_labels = mlsmote.fit_resample(X_train_tfidf, train_labels_float)
                print(f"Data shape after MLSMOTE: {X_resampled_tfidf.shape}, {y_resampled_labels.shape}")

                # Convert labels back to original type (int or bool) if needed
                y_resampled_labels = y_resampled_labels.astype(train_labels.dtype)


                # 3. Find closest original text for synthetic samples
                print("Finding closest original texts for synthetic samples...")
                n_original = X_train_tfidf.shape[0]
                n_synthetic = X_resampled_tfidf.shape[0] - n_original

                if n_synthetic > 0:
                    synthetic_tfidf = X_resampled_tfidf[n_original:]

                    # Use NearestNeighbors to find the closest original TF-IDF vector
                    nn = NearestNeighbors(n_neighbors=1, metric='cosine', algorithm='brute', n_jobs=-1)
                    nn.fit(X_train_tfidf)
                    distances, indices = nn.kneighbors(synthetic_tfidf)

                    # Get the indices of the closest original samples
                    closest_original_indices = indices.flatten()

                    # Create the resampled text list
                    # Ensure train_texts is indexable (e.g., list or pd.Series)
                    if isinstance(train_texts, pd.Series):
                        original_texts_list = train_texts.tolist()
                    else:
                        original_texts_list = list(train_texts) # Assume list-like

                    synthetic_texts = [original_texts_list[i] for i in closest_original_indices]

                    # Combine original texts with the new "synthetic" texts (which are copies of originals)
                    # Convert original texts to list if it's a series
                    resampled_texts_list = original_texts_list + synthetic_texts
                    # Convert back to Series if needed, maintaining index alignment might be tricky
                    # For simplicity, we'll use the list directly for the dataset
                    train_texts = pd.Series(resampled_texts_list) # Recreate series for dataset init
                    train_labels = y_resampled_labels # Use the labels generated by MLSMOTE

                    print(f"Combined {n_original} original and {n_synthetic} SMOTE-guided samples.")
                    print(f"New training set size: {len(train_texts)}")
                else:
                    print("No synthetic samples were generated by MLSMOTE.")
                    # Still need to ensure labels are in the correct format if no resampling happened
                    train_labels = train_labels_float.astype(train_labels.dtype)


            except Exception as e:
                print(f"Error during TF-IDF+SMOTE: {e}")
                print("Skipping SMOTE balancing due to error.")
                # Ensure labels are still in the original format if SMOTE failed
                # This might already be handled if the error was late, but good practice
                train_labels = train_labels # Keep original labels if error occurred


    elif args.balancing_strategy == 'random_oversample':
         # Use the random oversampling function
         train_texts, train_labels = random_oversample(train_texts, train_labels)
         # Ensure train_texts is a Series for the Dataset
         if not isinstance(train_texts, pd.Series):
              train_texts = pd.Series(train_texts)
...
if __name__ == "__main__":
    ...
    # Add balancing strategy argument
    parser.add_argument('--balancing_strategy', type=str, choices=['none', 'tfidf_smote', 'random_oversample'], default='none',
                        help='Strategy to balance the training data: '
                             'none=no balancing, '
                             'tfidf_smote=Use TF-IDF and MLSMOTE to guide oversampling (Experimental), '
                             'random_oversample=Duplicate minority class samples randomly')
