...
def main(args):
    ...
    # Prepare train data with optional token filtering
    ...
    all_labels = list(train_filtered_labels) + list(test_filtered_labels)
    mlb = MultiLabelBinarizer()
    mlb.fit(all_labels)
    print(f"Number of unique labels: {len(mlb.classes_)}")
    print("Labels:", mlb.classes_)
    
    # Set final train and test data
    ...
    all_labels_for_stats = sorted(set(train_label_dist.keys()) | set(test_label_dist.keys()))
    print("\nLabel          Train  Test")
    print("-" * 30)
