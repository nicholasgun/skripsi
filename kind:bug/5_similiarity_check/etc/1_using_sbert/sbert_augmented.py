...
def find_similar_requests(test_embeddings, reference_embeddings, reference_labels, top_k=5):
    """
    Find similar requests using cosine similarity.
    
    Args:
        test_embeddings: numpy array of shape (n_test, embedding_dim)
        reference_embeddings: numpy array of shape (n_reference, embedding_dim)
        reference_labels: list of label lists from reference data
...
def calculate_label_based_metrics(test_labels, reference_labels, similar_indices, k_values=[1, 3, 5]):
    """
    ...
    for i, test_label_set in enumerate(test_labels):
        ...
        for j, idx in enumerate(retrieved_indices):
            reference_labels_set = set(reference_labels[idx])
            matching_labels = test_labels_set & reference_labels_set  # Intersection
            ...
            if matching_labels:  # If there are matching labels
                ...
                overlap = len(matching_labels) / len(test_labels_set | reference_labels_set)
...
def load_and_process_csv(file_path, text_column='all_text'):
    """
    Load and process a CSV file, converting labels to proper format.
    ...
    Returns:
        pandas.DataFrame: Processed dataframe with converted labels
    """
    ...
    if text_column not in df.columns:
        ...
        print(f"Text column '{text_column}' not found. Available text columns: {available_columns}")
...
def main(args):
    ...
    # Load train and test data using the new function
    train_df = load_and_process_csv(args.train_data_path, text_column=args.text_column)
    test_df = load_and_process_csv(args.test_data_path, text_column=args.text_column)
    
    # Check if train data has 'is_augmented' column and separate data
    if 'is_augmented' in train_df.columns:
        print(f"\nFound 'is_augmented' column in training data")
        print(f"Total training samples: {len(train_df)}")
        
        # Count augmented vs original data
        augmented_count = train_df['is_augmented'].sum()
        original_count = len(train_df) - augmented_count
        print(f"Original samples: {original_count}")
        print(f"Augmented samples: {augmented_count}")
        
        # Create reference dataset with only original data (is_augmented == False)
        reference_df = train_df[train_df['is_augmented'] == False].copy()
        print(f"Reference dataset (original only): {len(reference_df)} samples")
        
        # Keep all data for training (original + augmented)
        print(f"Training dataset (original + augmented): {len(train_df)} samples")
    else:
        print("\nWarning: 'is_augmented' column not found in training data")
        print("Using all training data as both training and reference data")
    ...
    # Prepare train data with optional token filtering (use all data for training)
    print("\nPreparing training data (original + augmented)...")
    ...
    # Prepare reference data with the same filtering parameters (original data only)
    print("\nPreparing reference data (original only)...")
    reference_texts, reference_filtered_labels = prepare_data(
        reference_df, 
        text_column=args.text_column,
        min_label_freq=args.min_label_freq, 
        max_label_len=args.max_label_len,
    ...
    # Prepare test data with the same filtering parameters
    print("\nPreparing test data...")
    ...
    # Combine all labels to create a unified label encoder
    print("\nInitializing label encoder with combined labels...")
    all_labels = list(train_filtered_labels) + list(reference_filtered_labels) + list(test_filtered_labels)
    mlb = MultiLabelBinarizer()
    mlb.fit(all_labels)
    print(f"Number of unique labels: {len(mlb.classes_)}")
    print("Labels:", mlb.classes_)
    
    # Set final train, reference, and test data
    train_labels = train_filtered_labels.tolist()
    reference_labels = reference_filtered_labels.tolist()
    test_labels = test_filtered_labels.tolist()
    ...
    reference_label_dist = Counter([label for labels in reference_labels for label in labels])
    test_label_dist = Counter([label for labels in test_labels for label in labels])
    
    print("\nLabel distribution in datasets:")
    all_labels_for_stats = sorted(set(train_label_dist.keys()) | set(reference_label_dist.keys()) | set(test_label_dist.keys()))
    print("\nLabel              Train  Ref   Test")
    print("-" * 40)
    for label in all_labels_for_stats:
        train_count = train_label_dist.get(label, 0)
        reference_count = reference_label_dist.get(label, 0)
        test_count = test_label_dist.get(label, 0)
        print(f"{label[:15]:17} {train_count:5d} {reference_count:5d} {test_count:5d}")

    print(f"\nTotal samples - Training: {len(train_texts)}, Reference: {len(reference_texts)}, Testing: {len(test_texts)}")
    
    # Show augmentation impact on label distribution
    if 'is_augmented' in train_df.columns:
        print(f"\nAugmentation Impact:")
        print(f"Reference (original) has {len(reference_texts)} samples with {len(reference_label_dist)} unique labels")
        print(f"Training (orig+aug) has {len(train_texts)} samples with {len(train_label_dist)} unique labels")
        augmentation_ratio = len(train_texts) / len(reference_texts) if len(reference_texts) > 0 else 0
    ...
    # Generate embeddings for training and reference sets
    print("\nGenerating embeddings with model...")
    train_embeddings = get_embeddings(train_texts, model, batch_size=args.batch_size)
    reference_embeddings = get_embeddings(reference_texts, model, batch_size=args.batch_size)
    test_embeddings = get_embeddings(test_texts, model, batch_size=args.batch_size)
    
    # Find similar requests for each test sample using reference data (original only)
    print("\nFinding similar requests using reference data (original only)...")
    similar_indices, similarity_scores = find_similar_requests(
        test_embeddings, reference_embeddings, reference_labels, top_k=args.top_k
    )
    
    # Calculate label-based metrics using reference data
    print("\nCalculating label-based metrics...")
    k_values = [1, 3, 5, 10]  # Calculate precision@1, @3, @5, @10
    label_metrics = calculate_label_based_metrics(test_labels, reference_labels, similar_indices, k_values)
    ...
    for i, (indices, scores) in enumerate(zip(similar_indices, similarity_scores)):
        ...
        for j, (idx, score) in enumerate(zip(indices, scores)):
            reference_labels_set = set(reference_labels[idx])
            matching_labels = list(test_labels_set & reference_labels_set)
            
            similar_request = {
                'rank': j + 1,
                'text': reference_texts.iloc[idx],
...
if __name__  == "__main__":
    ...
    parser.add_argument('--train_data_path', type=str, 
                        default="/kaggle/input/preprocessed-feature-augmented-0-9/final_preprocessed_train_data.csv",
                        help='Path to the training CSV data file')
    parser.add_argument('--test_data_path', type=str, 
                        default="/kaggle/input/preprocessed-feature-augmented-0-9/final_preprocessed_val_data.csv",
